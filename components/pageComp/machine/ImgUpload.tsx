...
const ImgUpload = ({
  ...
  useEffect(() => {
    if (mode === "modify" && State !== "change") {
      if (
        item?.attached_imgurl !== null &&
        item?.attached_imgurl !== undefined
      ) {
        setImage({
  ...
  const deleteImage = async () => {
    ...
    if (mode === "modify") {
      ...
      const result = await MachineApplyImgPost(formData);
      setState("change");
    }
    if (mode !== "modify") {
      setSendImg !== undefined &&
        sendImg !== undefined &&
        setSendImg(
          sendImg.filter((el: ISendImg) => el.id !== item.attached_number)
        );
    }
  };
          ...
          <div className="inner">
            <>
              {image.preview_URL !== "" ? (
                ...
                <div>이미지-{item.attached_number}</div>
              )}
            </>
          </div>
        ) : (
