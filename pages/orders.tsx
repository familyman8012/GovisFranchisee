...
import {
  IOrdersListRow,
  IOrdersRequestDateRange,
  ITotal,
} from "InterfaceFarm/Order";
...
export default function Orders() {
  ...
  const [channelTypeName, setChannelTypeName] = useState<any>(
    getChannelTypeByChannelKey()
  );
  ...
  const getOrders = async (isInit: boolean) => {
    let listView: Array<IOrdersListRow> | undefined =
      [] as Array<IOrdersListRow>;
  ...
  return (
    <Layout className="fullWidth">
      ...
      <Container className={"contents orders"}>
        <Row>
          <Col>
            <SummaryList>
              {list &&
                list.map((data) => (
                  <SummaryItemForOrder key={data.order_number} data={data} />
                ))}
            </SummaryList>
          </Col>
        </Row>
        <Row className="my-3">
          <Col>
            {total.total_order_count > list.length && (
