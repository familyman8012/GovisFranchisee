...
import { iInteriorListContent, iInteriorReview, iInteriorViewContent } from "InterfaceFarm/InteriorBoard";
import { SyntheticEvent } from "react";
import InteriorComplete from "./InteriorComplete";

interface InteriorContentsListProps {
  review: null | iInteriorReview;
  dataList: iInteriorListContent[];
  onClickDelete?: (sbic_idx: number) => void;
}

const getProcData = (dataList: iInteriorViewContent[]) => {
  let reVal = [];
  let lastCreateDate = "";
...
export const InteriorBoardContentsList: React.FC<InteriorContentsListProps> = ({ review, dataList, onClickDelete }) => {
  ...
  return (
    <>
      <ul className={"contents-list"}>
        ...
        {viewData.map((row) => {
          if (row.content_type === -1) {
            return (
              <li className="date no-background" key={row.sbic_idx}>
            ...
            return (
              <li key={row.sbic_idx} className="no-background">
                <Row>
            ...
            return (
              <li className="answer" key={row.sbic_idx}>
                <h6>담당자</h6>
                <p>{row.content_value}</p>
              </li>
            );
            ...
            return (
              <li className="no-background answer" key={row.sbic_idx}>
                <Row>
                  <div className="col-sm-10 col-9">
                    <img src={row.content_value} width="100%" onError={getImageErrorNoImage} />
                  </div>
      ...
      {review && <InteriorComplete review={review} />}
    </>
  );
};
