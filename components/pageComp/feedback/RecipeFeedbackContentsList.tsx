...
import { ChevronRight, Pencil, Trash } from "@emotion-icons/bootstrap";
import { IRecipeFeedbackViewContentsItem } from "InterfaceFarm/ProductFeedback";
import { EmptyView } from "ComponentsFarm/elements/EmptyView";
import { Col, Row } from "ComponentsFarm/layouts/styles";

interface RecipeFeedbackContentsListProps {
  dataList: IRecipeFeedbackViewContentsItem[];
  onClickDelete?: (sbfc_idx: number) => void;
}

const getProcData = (dataList: IRecipeFeedbackViewContentsItem[]) => {
  let reVal = [];
  let lastCreateDate = "";
...
export const RecipeFeedbackContentsList: React.FC<RecipeFeedbackContentsListProps> = ({ dataList, onClickDelete }) => {
    ...
    <ul className={"contents-list"}>
      ...
      {viewData.map((row) => {
        if (row.content_type === -1) {
          return (
            <li className={"date no-background"} key={row.sbfc_idx}>
          ...
          return (
            <li key={row.sbfc_idx} className={"no-background"}>
              <Row>
          ...
          return (
            <li className={"answer"} key={row.sbfc_idx}>
              <h6>담당자</h6>
              <p>{row.content_value}</p>
            </li>
          );
          ...
          return (
            <li className={"no-background answer "} key={row.sbfc_idx}>
              <Row>
                <div className="col-sm-10 col-9">
                  <img src={row.content_value} width={"100%"} onError={getImageErrorNoImage} />
                </div>
