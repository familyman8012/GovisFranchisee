...
import { iReturnListContent, iReturnViewContent } from "InterfaceFarm/ReturnBoard";
import { SyntheticEvent } from "react";
import { NoticeComplete } from "../interior/style";

interface ReturnContentsListProps {
  status: number;
  dataList: iReturnListContent[];
  onClickDelete?: (sbrec_idx: number) => void;
}

const getProcData = (dataList: iReturnViewContent[]) => {
  let reVal = [];
  let lastCreateDate = "";
...
export const ReturnBoardContentsList: React.FC<ReturnContentsListProps> = ({ status, dataList, onClickDelete }) => {
  ...
  return (
    <>
      <ul className={"contents-list"}>
        ...
        {viewData.map((row) => {
          if (row.content_type === -1) {
            return (
              <li className="date no-background" key={row.sbrec_idx}>
                <h5>{row.content_value}</h5>
              </li>
            );
          } else if (row.content_type === 0 && row.user_type === 1) {
            return (
              <li key={row.sbrec_idx}>
            ...
            return (
              <li key={row.sbrec_idx} className="no-background">
                <Row>
            ...
            return (
              <li className="answer" key={row.sbrec_idx}>
                <h6>담당자</h6>
                <p>{row.content_value}</p>
              </li>
            );
            ...
            return (
              <li className="no-background answer" key={row.sbrec_idx}>
                <Row>
                  <div className="col-sm-1 col-3 offset-sm-1">
                    <img src={row.content_value} width="100%" onError={getImageErrorNoImage} />
                  </div>
      ...
      {status === 2 && <NoticeComplete>처리가 완료된 요청건입니다</NoticeComplete>}
    </>
  );
};
