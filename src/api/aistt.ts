...
export const fetchMonitoringStoreRecordList = () => {
  ...
  >(`/aifqs/monitoring/table/record`).then((res) => res.data.data);
};
...
export const fetchMonitoringStoreVideoList = ({
  ...
  >(`/aifqs/monitoring/table/record/list/${record_date}`).then(
    (res) => res.data.data
  );
};
...
export const fetchMonitoringStoreProductList = ({
  ...
  >(
    `/aifqs/monitoring/table/record/list/${record_date}/${store_stt_cctv_idx}/inspection`,
    {
      params: {
        current_num: 1,
        per_num: 9999,
