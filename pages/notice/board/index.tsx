...
import { INoticeBoardListRow, INoticeBoardListRequest, INoticeBoardListResponse } from "InterfaceFarm/Notice";
...
export default function NoticeList() {
  ...
  const user_id = useMemo(() => session?.info?.user_id ?? -1, [session?.info?.user_id]);

  const [params, setParams] = useQueryString<Omit<INoticeBoardListRequest, "user_id">>({
    page: parseInt((qs.get("page") as string) ?? "1"),
    size: 10,
    search: qs.get("search") ?? "",
  });

  const isInitialize = useMemo(() => list.length === 0 && params.page > 1, [list, params]);
  ...
  const handlerBtnMore = React.useCallback(() => setParams({ ...params, page: params.page + 1 }), [params]);
  ...
  return (
    <Layout>
      ...
      <div className={`p-0 pb-5 ${style["notice"]}`}>
        <NoticeListHeader initialKeyword={params.search} callbackSearch={handlerSearch} />
        <NoticeBoardList
          className="orders"
          notices={list}
          loading={isLoading}
        ...
        />
        {list.length !== totalCount && <ListMoreButton handler={handlerBtnMore} />}
      </div>
    </Layout>
  );
}
